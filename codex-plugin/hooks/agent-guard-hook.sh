#!/usr/bin/env bash
# Agent Guard — OpenAI Codex CLI tool-call screening hook command.
#
# Registered for both PreToolUse and PostToolUse. Codex spawns this per tool
# call, hands it the event as JSON on stdin, and reads one JSON decision back
# on stdout. The screening engine is Python; this script is the thin seam that
# bridges to it. The bridge inspects `hook_event_name` and screens the tool
# input (PreToolUse) or the tool result (PostToolUse) accordingly.
#
